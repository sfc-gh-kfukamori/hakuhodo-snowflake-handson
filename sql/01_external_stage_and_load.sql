...
CREATE STORAGE INTEGRATION IF NOT EXISTS GCS_HAKUHODO_INTEGRATION
    TYPE = EXTERNAL_STAGE
    STORAGE_PROVIDER = 'GCS'
    ENABLED = TRUE
    STORAGE_ALLOWED_LOCATIONS = (
        'gcs://<YOUR_GCS_BUCKET_NAME>/<YOUR_PATH>/'   -- ★ 実際のGCSバケットパスに置き換えてください
    );
...
CREATE FILE FORMAT IF NOT EXISTS HAKUHODO_HANDSON_DB.HAKUHODO_HANDSON_SCHEMA.CSV_FORMAT_JP
    TYPE = 'CSV'
    FIELD_DELIMITER = ','
    RECORD_DELIMITER = '\n'
    SKIP_HEADER = 1
...
CREATE STAGE IF NOT EXISTS HAKUHODO_HANDSON_DB.HAKUHODO_HANDSON_SCHEMA.GCS_HAKUHODO_STAGE
    STORAGE_INTEGRATION = GCS_HAKUHODO_INTEGRATION
    URL = 'gcs://<YOUR_GCS_BUCKET_NAME>/<YOUR_PATH>/'   -- ★ 実際のGCSパスに置き換えてください
