...
CREATE OR REPLACE AGENT HAKUHODO_HANDSON_DB.ANALYTICS.HAKUHODO_INTELLIGENCE
  ...
  instructions:
    orchestration: >
      ...
      DataAnalyst: 数値データ・実績・予算・仕入高・売上・KPI等の定量的な質問に使用。
      KnowledgeSearch: 社内ルール・ガイドライン・手順・マニュアル等の定性的な質問に使用。
      両方が必要な場合は、まずDataAnalystでデータを取得し、次にKnowledgeSearchでナレッジを検索して統合的に回答してください。
    response: >
      常に日本語で回答してください。数値を含む回答では具体的な数字を明示し、
      金額は読みやすい単位（万円、億円）で表示してください。
      根拠となるデータソース（テーブル名またはドキュメント名）を明記してください。
      不明な場合は推測せず、該当するデータが見つかりませんと回答してください。
    sample_questions:
      - question: "2024年度の媒体種類別の仕入高ランキングを教えて"
      - question: "予算達成率が最も低い部門はどこ？改善のためにどんな分析をすべき？"
      - question: "仕入の承認フローと決裁金額の基準を教えて"
      - question: "デジタル広告のKPI体系をファネル別に整理して"
      - question: "デジタル媒体の仕入高を調べて、運用基準に照らして評価して"
      - question: "業種別の仕入高データから、営業強化すべき業種とその戦略を提案して"
...
--
-- 比較・トレンド分析:
--   「2023年度と2024年度の仕入高を比較して」
--   「四半期ごとの媒体収益の推移を教えて」
--   「代理店系列（H, D, Y）別の収益率を比較して」
--
-- 予実管理・組織損益:
--   「予算達成率が最も低い部門はどこ？」
--   「博報堂DYメディアパートナーズの売上総利益の予実差異は？」
--   「部門グループ別の実績合計を降順で見せて」
--
-- 深掘り分析:
--   「広告主業種別で仕入高が最も大きい業種トップ5は？」
--   「マスメディア区分ごとの仕入高構成比を教えて」
--   「スタッフコストが最も高い四半期はいつ？」
--   「FC営業収入と媒体収益の相関を年度別に見せて」

-- ============================================================================
-- ★ カテゴリ2: 社内ナレッジ検索（KnowledgeSearch ツール）
-- ============================================================================
-- 媒体仕入ガイドライン:
--   「仕入金額の承認フローと決裁基準を教えて」
--   「テレビスポットの仕入で部長承認が必要になるのはどんなケース？」
--   「デジタル媒体のビューアビリティ率の最低基準は何%？」
--   「緊急案件で事後承認が認められる条件は？」
--
-- 予算管理マニュアル:
--   「予算策定のスケジュールを月ごとに教えて」
--   「予実差異率が10%を超えた場合に必要なアクションは？」
--   「フォーキャスト（ローリング予測）はいつ更新する？」
--   「差異分析で見るべき5つの視点を教えて」
--
-- デジタル広告運用基準:
--   「デジタル広告のKPI体系をファネル別に教えて」
--   「Google広告のキーワード品質スコアの最低基準は？」
--   「Meta広告のクリエイティブテストは何パターン推奨？」
--   「ROASの推奨基準は何%以上？」
--
-- 営業戦略:
--   「自動車業種の広告投資トレンドと営業戦略のポイントは？」
--   「食品・飲料業種での博報堂DYグループのシェアは？」
--   「クライアントへの提案時の基本フレームワークを教えて」
--   「クロスセル推進のために仕入データをどう活用する？」
--
-- Snowflake活用:
--   「社内のSnowflake環境のデータベース構成を教えて」
--   「DT_PURCHASE_MONTHLY_SUMMARYテーブルでは何が分かる？」
--   「Snowflake Intelligenceへのアクセス方法を教えて」

-- ============================================================================
-- ★ カテゴリ3: 両ツール横断（Analyst + Search の組み合わせ）
-- ============================================================================
-- データ × ガイドライン:
--   「デジタル媒体の2024年度仕入高を確認し、運用基準に照らして評価して」
--   「仕入高が1億円を超えている媒体種類を調べて、それぞれの承認フローを教えて」
--   「テレビ媒体の仕入高推移と、テレビ仕入の判断基準を合わせて教えて」
--
-- データ × 予算管理:
--   「予算達成率が最も低い部門を特定し、差異分析で見るべき視点を教えて」
--   「Q2時点の実績から年度着地見込みの考え方と、予算比マイナス15%超の場合の対応を教えて」
--
-- データ × 営業戦略:
--   「業種別の仕入高データを見て、博報堂のシェアが低い業種の営業戦略を提案して」
--   「媒体種類の偏りがある代理店系列を特定し、クロスセルの機会を教えて」
--
-- 総合的な経営判断:
--   「2024年度の全体KPIサマリーを出し、改善が必要な領域についてナレッジから施策を提案して」
--   「収益率が低い媒体種類を特定し、その媒体の仕入基準とKPI目安を合わせて教えて」
-- ============================================================================
